...
class SchemaToGraph
  ...
  def evaluate_schema_simple!
    ...
    @schema_rb.each_line do |raw|
      ...
      if current_table
        ...
        # Support both symbol and string column names
  ...
  def sanitize_schema(str)
    ...
    # Remove entire index/check_constraint statements
  ...
  def layout_nodes!(nodes, links)
    ...
    # Estimate box sizes similar to the JS renderer
    padx = 16.0
    row_h = 26.0
    hdr_h = 30.0
    type_w = 82.0
    ...
    # Golden-angle spiral layout: evenly spread in all directions
    golden = 2.399963229728653
    avg_dim = nodes.map { |n| [n[:w], n[:h]].max }.sum / [nodes.length, 1].max
    c = (avg_dim * 0.6) + 60.0 # radial step constant
  ...
  class Shim
    ...
    def dsl
      <<~RUBY
        ...
        # Swallow any stray top-level calls
